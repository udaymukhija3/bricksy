...
export interface State { player: P; crate: Crate }
...
export const stateKey = (s: State) => `${key(s.player)}|${key(s.crate.pivot)}|${s.crate.rot & 3}`;
...
export function apply(level: Level, s: State, m: Move): State | null {
  ...
  const free = (p: P) => p[0] >= 0 && p[1] >= 0 && p[0] < level.w && p[1] < level.h && !walls.has(key(p));
  ...
  if (cells.some((c) => same(c, np))) {
    ...
    return { player: np, crate: { pivot: add(s.crate.pivot, d), rot: s.crate.rot } };
  }
  return { player: np, crate: s.crate };
}

export const solved = (level: Level, s: State) => {
  const cells = crateCells(level.shape, s.crate).map(key).sort().join(';');
  return cells === level.socket.map(key).sort().join(';');
};
...
export interface Spec { w: number; h: number; obstacles: number; shape: keyof typeof SHAPES; parMin: number; parMax: number; needTurn: boolean }

/** Difficulty by level: bigger rooms, more clutter, longer solutions, then a crate that must be turned. */
export function spec(level: number): Spec {
  ...
  if (level < 8) return { w: 7, h: 7, obstacles: 4, shape: 'L3', parMin: 9, parMax: 11, needTurn: true };
  return { w: 8, h: 8, obstacles: 5, shape: level % 2 ? 'L4' : 'T4', parMin: 10, parMax: 14, needTurn: true };
}
...
export function makeLevel(sp: Spec, rng: () => number): Level {
  ...
  for (let tries = 0; tries < 300; tries++) {
    ...
    const base: Level = { w, h, walls: [...walls], shape, start: { player, crate }, socket: [], par: 0, solution: [] };
    // Distinct crate configurations by their fewest-moves distance (over all player positions).
    const reach = explore(base);
    const byConfig = new Map<string, Node>();
    for (const n of reach.values()) {
      const k = `${key(n.s.crate.pivot)}|${n.s.crate.rot & 3}`;
      const cur = byConfig.get(k);
      if (!cur || n.dist < cur.dist) byConfig.set(k, n);
    }
    const goals = [...byConfig.values()].filter((n) => n.dist >= sp.parMin && n.dist <= sp.parMax && (!sp.needTurn || (n.s.crate.rot & 3) !== (crate.rot & 3)));
    if (!goals.length) continue;
    const goal = goals[rnd(goals.length)];
    const socket = crateCells(shape, goal.s.crate);
    const level: Level = { ...base, socket, par: goal.dist, solution: pathTo(goal) };
